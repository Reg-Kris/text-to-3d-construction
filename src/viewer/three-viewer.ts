...
export class ThreeViewer {
  ...
  private initRenderer(config: ViewerConfig): void {
    this.renderer = new THREE.WebGLRenderer({ 
      antialias: this.deviceOptimizer.getOptimizationSettings().enableAntialiasing,
      alpha: true,
  ...
  private initScene(config: ViewerConfig): void {
  ...
  async loadModel(url: string, format: 'glb' | 'fbx' | 'obj' = 'glb', onProgress?: (progress: number) => void): Promise<ModelInfo> {
    return new Promise((resolve, reject) => {
      ...
      switch (format) {
        case 'glb':
          this.loadGLTFModel(url, successCallback, progressCallback, errorCallback);
          break;
        case 'fbx':
          this.loadFBXModel(url, successCallback, progressCallback, errorCallback);
          break;
        case 'obj':
          this.loadOBJModel(url, successCallback, progressCallback, errorCallback);
          break;
        default:
  ...
  private async loadGLTFModel(
    ...
    this.gltfLoader.load(url, (gltf: any) => successCallback(gltf.scene), progressCallback, errorCallback);
  }
